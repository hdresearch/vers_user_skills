...
VM=$(curl -sH "$H" -H 'Content-Type: application/json' \
     -d '{"vm_config":{"vcpu_count":<N>,"mem_size_mib":<MiB>,"fs_size_mib":<MiB>,
...
CID=$(curl -sH "$H" -H 'Content-Type: application/json' \
     -d '{"name":"<commit-name>","description":"<description>"}' \
...
curl -sH "$H" -H 'Content-Type: application/json' \
     -d '{"name":"<repo>","description":"<description>"}' "$B/repositories"
curl -sH "$H" -H 'Content-Type: application/json' \
     -d "{\"tag_name\":\"carters_fave_compilers_and_provers\",\"commit_id\":\"$CID\"}" \
     "$B/repositories/<repo>/tags"
```
...
curl -sH "$H" -X POST \
    "$B/vm/branch/by_ref/<repo>/<tag>?count=<N>&keep_paused=true" | jq '.vms[].vm_id'

# By legacy org-scoped tag
curl -sH "$H" -X POST "$B/vm/branch/by_tag/<tag>?count=<N>" | jq -r .vms[0].vm_id

# By commit id directly
curl -sH "$H" -X POST "$B/vm/branch/by_commit/$CID?count=<N>" | jq .
```
